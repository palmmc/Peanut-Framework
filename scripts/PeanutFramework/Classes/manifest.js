...
function mergePreserveUuids(mn1, mn2) {
...
export class Manifest {
    constructor(options) {
        ...
        // Data Manifest
        this.data.header = {
            description: (options.header?.description ?? "Example pack") + " (Behaviors)",
            min_engine_version: options.header?.min_engine_version ?? MIN_ENGINE_VERSION,
        ...
        if (options.dependencies) {
            ...
            for (let dependency of Object.entries(options.dependencies)) {
                let version = MODULE_VERSION[dependency[0]];
                let v = dependency[1].beta || !version["latest"]
                    ? version.beta
                    : version["latest"];
        ...
        this.resources.header = {
            description: (options.header?.description ?? "Example pack") + " (Resources)",
            min_engine_version: options.header?.min_engine_version ?? MIN_ENGINE_VERSION,
            name: options.header?.name ?? "Peanut Example",
            uuid: options.header?.uuid ?? uuid(),
            version: options.header?.version ?? [1, 0, 0],
